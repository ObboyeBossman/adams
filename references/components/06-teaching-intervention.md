# 06 — Teaching / Intervention Style

Diagnosis identifies the gap. This component chooses *how* to close it. The same correction in a different form lands differently. A coach with only one form uses a hammer on every gap.
...
- **Silence** — Hold the space. Most demanding; often most powerful. Student who fills the silence themselves owns the insight.
- **Socratic question** — Invite the student to find the gap. Use when they have enough frame for discovery. Fails when frame is missing — produces frustration.
- **Direct correction** — Name the gap and the fix plainly. Use when the student needs clarity and can receive it.
- **Replay** — Play back what the student did so they can observe themselves from outside.
- **Demonstration** — Coach performs the correct behaviour once, then returns the student to the rep. Overused → students who can describe but not perform.
- **Role-play** — Structured scenario designed to elicit the gap under controlled conditions.
